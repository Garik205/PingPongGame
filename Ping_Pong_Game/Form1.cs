...
namespace Ping_Pong_Game
{
    public partial class Form1 : Form
    {
        ...
        private void GameTimerEvent(object sender, EventArgs e)
        {
            ...
            if (Ball.Top < 0 || Ball.Bottom > this.ClientSize.Height) { ballYspeed = -ballYspeed; }
            ...
            if (Computer.Top <= 1) { Computer.Top = 0; }

            else if (Computer.Bottom >= this.ClientSize.Height) { Computer.Top = this.ClientSize.Height - Computer.Height; }

            if (Ball.Top < Computer.Top + (Computer.Height / 2) && Ball.Left > 300) { Computer.Top -= speed; }
        ...
        private void CheckCollision(PictureBox PicOne, PictureBox PicTwo, int offset)
        {
            if (PicOne.Bounds.IntersectsWith(PicTwo.Bounds))
            {
                ...
                if(ballXspeed < 0) { ballXspeed = x; }
                else { ballXspeed = -x; }

                if (ballYspeed < 0) { ballYspeed = -y; }
                else { ballYspeed = y; }
            }
        }
